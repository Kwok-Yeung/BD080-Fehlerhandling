...
CLASS lcx_exception_class DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS:
      BEGIN OF c_dynamic_message,
        msgid TYPE symsgid VALUE '00',
        msgno TYPE symsgno VALUE '398',
        attr1 TYPE scx_attrname VALUE 'MF_MSGV1',
        attr2 TYPE scx_attrname VALUE 'MF_MSGV2',
        attr3 TYPE scx_attrname VALUE 'MF_MSGV3',
        attr4 TYPE scx_attrname VALUE 'MF_MSGV4',
      END OF c_dynamic_message .
    ...
    "Attributes for dynamic messages.
    DATA mf_dynamic_mess TYPE abap_bool .
    DATA mf_msgid TYPE sy-msgid .
    DATA mf_msgno TYPE sy-msgno .
    DATA mf_msgv1 TYPE sy-msgv1 .
    DATA mf_msgv2 TYPE sy-msgv2 .
    DATA mf_msgv3 TYPE sy-msgv3 .
    DATA mf_msgv4 TYPE sy-msgv4 .
    DATA mf_msgty TYPE sy-msgty .
    ...
    METHODS constructor
      IMPORTING
        ...
        !if_dynamic_mess TYPE abap_bool OPTIONAL
        !if_msgid        TYPE sy-msgid OPTIONAL
        !if_msgno        TYPE sy-msgno OPTIONAL
        ...
        !if_msgty        TYPE sy-msgty OPTIONAL .

    METHODS do_fill_syst_variables .
...
CLASS lcl_exception_class DEFINITION FINAL.
  ...
  PRIVATE SECTION.
    ...
    "Help methods.
    METHODS help_raise_exception_matnr
      RAISING
        lcx_exception_class.
...
CLASS lcl_exception_class IMPLEMENTATION.
  METHOD le_catch_class_exception.
    ...
    DATA lo_x TYPE REF TO lcx_exception_class.

    BREAK-POINT.

    "Call method which raises a static exception.
    TRY.
        me->help_raise_exception_matnr( ).
      CATCH lcx_exception_class INTO lo_x.
        CLEAR: sy-msgty, sy-msgid, sy-msgno, sy-msgv1, sy-msgv2, sy-msgv3, sy-msgv4.
        "Fill system variables.
        lo_x->do_fill_syst_variables( ).
        "Send message.
        MESSAGE ID sy-msgid TYPE 'I' NUMBER sy-msgno
          WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
        "Get message text.
    ...
    "Call method which raises the dynamic exception.
    TRY.
        me->help_raise_exception_dyn( ).
      CATCH lcx_exception_class INTO lo_x.
        CLEAR: sy-msgty, sy-msgid, sy-msgno, sy-msgv1, sy-msgv2, sy-msgv3, sy-msgv4.
        "This does not make sense for the dynamic exceptions,
        "because exception C_DYNAMIC_MESSAGE uses dummy values for MSGID and MSGNO.
        "The original MSGID and MSGNO are saved in attributes MF_MSGID and MF_MSGID,
        "which the standard does not know.
*        lf_string = lo_x->get_text( ).
  ...
  METHOD help_raise_exception_dyn.
    ...
    MESSAGE e042(06) WITH '4711' 13 'EUR' INTO lf_string.

    "Raise dynamic class based exception.
    RAISE EXCEPTION TYPE lcx_exception_class
      EXPORTING
        is_textid       = lcx_exception_class=>c_dynamic_message
        if_dynamic_mess = abap_true
        if_msgid        = sy-msgid
        if_msgno        = sy-msgno
        if_msgv1        = sy-msgv1
        if_msgv2        = sy-msgv2
        if_msgv3        = sy-msgv3
        if_msgv4        = sy-msgv4.
  ENDMETHOD.                    "help_raise_exception_dyn

  METHOD help_raise_exception_matnr.
    "Raise class based exception.
*
*    RAISE EXCEPTION TYPE zcx_csu
*      EXPORTING
*        textid = zcx_csu=>material_not_exists
*        mf_matnr  = '4711'.
    ...
    RAISE EXCEPTION TYPE lcx_exception_class
      EXPORTING
        ...
        if_matnr  = '4711'.
  ENDMETHOD.                    "help_raise_exception_matnr
...
CLASS lcx_exception_class IMPLEMENTATION.
  METHOD constructor ##adt_suppress_generation.
    ...
    "Fill attributes for dynamic messages.
    me->mf_dynamic_mess = if_dynamic_mess .
    me->mf_msgid = if_msgid .
    me->mf_msgno = if_msgno .
    me->mf_msgv1 = if_msgv1 .
    me->mf_msgv2 = if_msgv2 .
    me->mf_msgv3 = if_msgv3 .
    me->mf_msgv4 = if_msgv4 .
    me->mf_msgty = if_msgty .
...
*      CLEAR me->ms_textid.
  ...
  METHOD do_fill_syst_variables.
    "Fill message variables: SY-MSGTY, SY-MSGID, SY-MSGNO, SY-MSGV1, ...
    DATA lf_msgv1 TYPE sy-msgv1.
    DATA lf_msgv2 TYPE sy-msgv2.
    DATA lf_msgv3 TYPE sy-msgv3.
    DATA lf_msgv4 TYPE sy-msgv4.
    DATA lf_string TYPE string.
    FIELD-SYMBOLS <lf_any> TYPE any.

    IF mf_dynamic_mess = abap_true.
      MESSAGE ID     me->mf_msgid
              TYPE   'E'
              NUMBER me->mf_msgno
              WITH   me->mf_msgv1
                     me->mf_msgv2
    ...
    ELSE.
      ...
      IF me->if_t100_message~t100key-attr1 IS NOT INITIAL.
        ASSIGN (me->if_t100_message~t100key-attr1) TO <lf_any>.
        IF sy-subrc = 0.
          lf_msgv1 = <lf_any>.
        ENDIF.
      ENDIF.
      IF me->if_t100_message~t100key-attr2 IS NOT INITIAL.
        ASSIGN (me->if_t100_message~t100key-attr2) TO <lf_any>.
        IF sy-subrc = 0.
          lf_msgv2 = <lf_any>.
        ENDIF.
      ENDIF.
      IF me->if_t100_message~t100key-attr3 IS NOT INITIAL.
        ASSIGN (me->if_t100_message~t100key-attr3) TO <lf_any>.
        IF sy-subrc = 0.
          lf_msgv3 = <lf_any>.
        ENDIF.
      ENDIF.
      IF me->if_t100_message~t100key-attr4 IS NOT INITIAL.
        ASSIGN (me->if_t100_message~t100key-attr4) TO <lf_any>.
        IF sy-subrc = 0.
          lf_msgv4 = <lf_any>.
        ENDIF.
      ENDIF.
    ...
    IF lf_string IS INITIAL.
      CLEAR lf_string.
    ENDIF.
  ENDMETHOD.                    "do_fill_syst_variables
...
CLASS lcl_show_collect_mess IMPLEMENTATION.
  ...
  METHOD le_show_collect_mess.
    ...
    "Store a message.
    ADD 1 TO lf_line.
    MESSAGE w447(0d) WITH '0053027720' '1001001001' '4479000000' 'KA4' INTO lf_string.
    me->help_store_message_syst( if_line = lf_line ).

    "Store a message.
    ADD 1 TO lf_line.
    MESSAGE e450(06) WITH '0000020020' 'M4711' 13 'ST' INTO lf_string.
    me->help_store_message_syst( if_line = lf_line ).

    ADD 1 TO lf_line.
    MESSAGE w450(06) WITH '0000020020' 'M4711' 13 'ST' INTO lf_string.
    me->help_store_message_syst( if_line = lf_line ).
...
*   ADD 1 TO lf_line.
    ...
    "Store a message.
    ADD 1 TO lf_line.
    MESSAGE i208(00) WITH 'Ganz wichtige Info. Heute ist ... .' INTO lf_string.
    me->help_store_message_syst( if_line = lf_line ).
...
*   ADD 1 TO lf_line.
  ...
  METHOD le_show_collect_mess2.
    ...
    ADD 1 TO lf_line.
    MESSAGE i208(00) WITH '--- Überschrift 1, Auftrag 531 ---' INTO lf_string.
    ...
    ADD 1 TO lf_line.
    MESSAGE i208(00) WITH '--- Überschrift 2, Auftrag 532 ---' INTO lf_string.
    me->set_default_line_syst( if_line = lf_line ).
    ...
    "Store a message: Do not increment LF_LINE if message belongs to a headline.
    MESSAGE w029(m3) WITH '4711' '1000' INTO lf_string.
    me->help_store_message_syst( if_line = lf_line ).
    ...
    "---Store a headline. Headline with messages will not be shown.
    ADD 1 TO lf_line.
    MESSAGE i208(00) WITH '--- Überschrift 3, Auftrag 533 ---' INTO lf_string.
    me->set_default_line_syst( if_line = lf_line ).
    ...
    "---Store a headline.
    ADD 1 TO lf_line.
    MESSAGE i208(00) WITH '--- Überschrift 4, Auftrag 534 ---' INTO lf_string.
    me->set_default_line_syst( if_line = lf_line ).
    ...
    "---Store a headline.
    ADD 1 TO lf_line.
    MESSAGE s208(00) WITH '--- Überschrift 5, Auftrag 535 ---' INTO lf_string.
    me->set_default_line_syst( if_line = lf_line ).

    "Store a message: Do not increment LF_LINE if message belongs to a headline.
    MESSAGE s447(0d) WITH '0053027720' '1001001001' '4479000000' 'KA4' INTO lf_string.
    me->help_store_message_syst( if_line = lf_line ).
